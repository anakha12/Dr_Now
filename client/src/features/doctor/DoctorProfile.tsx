...
import { getDoctorProfile, updateDoctorProfile, getAllDepartments } from "../../services/doctorService";
...
const DoctorProfile = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        setForm({
          ...
          experience: profile.experience?.length ? profile.experience : [{ hospital: "", role: "", years: "" }],
        ...
        setLoading(false);
        addNotification("Failed to load profile or departments", "error");
      }
    };
  ...
  const handleArrayChange = (index: number, field: string, value: any, key?: string) => {
    const updatedArray = [...(form as any)[field]];
    if (key) {
  ...
  const validateForm = () => {
    ...
    if (!form.yearsOfExperience || form.yearsOfExperience < 0) newErrors.yearsOfExperience = "Enter valid experience";
    if (!form.specialization.trim()) newErrors.specialization = "Specialization is required";
    if (!form.gender.trim()) newErrors.gender = "Gender is required";
    if (!form.consultFee || isNaN(Number(form.consultFee))) newErrors.consultFee = "Valid consult fee is required";
    setErrors(newErrors);
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  try {
    ...
    if (res?.message === "confirmation_required") {
      const confirmed = await confirmMessage("Updating your profile will block your account temporarily. Proceed?");
      if (!confirmed) return;

      const res2 = await updateDoctorProfile({ ...form, confirm: true });

      if (res2?.message === "confirmation_required") {
        addNotification("Confirmation still required. Please try again.", "warning");
        return;
      }

      addNotification("Profile updated with confirmation", "success");
      setTimeout(() => {
        navigate("/user/login"); 
      }, 1500);
    } else {
      ...
      {!editMode ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 text-gray-800 text-base">
          ...
          <div className="sm:col-span-2">
            ...
            <ul className="list-disc ml-6 text-sm text-gray-700">{form.awards.map((a, i) => <li key={i}>{a}</li>)}</ul>
          </div>
          <div className="sm:col-span-2">
            <p><strong>Education:</strong></p>
            <ul className="list-disc ml-6 text-sm text-gray-700">
              {form.education.map((e, i) => <li key={i}>{e.degree}, {e.institution} ({e.year})</li>)}
            </ul>
          </div>
          <div className="sm:col-span-2">
            <p><strong>Experience:</strong></p>
            <ul className="list-disc ml-6 text-sm text-gray-700">
              {form.experience.map((e, i) => <li key={i}>{e.role} at {e.hospital} ({e.years})</li>)}
            </ul>
          </div>
          <div className="sm:col-span-2">
            <p><strong>Affiliated Hospitals:</strong></p>
            <ul className="list-disc ml-6 text-sm text-gray-700">{form.affiliatedHospitals.map((h, i) => <li key={i}>{h}</li>)}</ul>
          </div>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <div className="col-span-1 sm:col-span-2">
            ...
            {form.education.map((edu, i) => (
              <div key={i} className="grid grid-cols-3 gap-2 mb-2">
                <input value={edu.degree} onChange={(e) => handleArrayChange(i, "education", e.target.value, "degree")} placeholder="Degree" className="border p-2 rounded" />
                <input value={edu.institution} onChange={(e) => handleArrayChange(i, "education", e.target.value, "institution")} placeholder="Institution" className="border p-2 rounded" />
                <input value={edu.year} onChange={(e) => handleArrayChange(i, "education", e.target.value, "year")} placeholder="Year" className="border p-2 rounded" />
              </div>
            ))}
          ...
          <div className="col-span-1 sm:col-span-2">
            ...
            {form.experience.map((exp, i) => (
              <div key={i} className="grid grid-cols-3 gap-2 mb-2">
                <input value={exp.hospital} onChange={(e) => handleArrayChange(i, "experience", e.target.value, "hospital")} placeholder="Hospital" className="border p-2 rounded" />
                <input value={exp.role} onChange={(e) => handleArrayChange(i, "experience", e.target.value, "role")} placeholder="Role" className="border p-2 rounded" />
                <input value={exp.years} onChange={(e) => handleArrayChange(i, "experience", e.target.value, "years")} placeholder="Years" className="border p-2 rounded" />
              </div>
            ))}
